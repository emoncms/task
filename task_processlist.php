...
class Task_ProcessList {
    ...
    public function process_list() {
        ...
        $list[] = array(_("Feed last update > secs, go to next"), ProcessArg::VALUE, "feed_last_update_higher", 0, DataType::UNDEFINED, "Time", 'desc' => _("<p>Using the value of previous process as a feed id, the execution of the processlist will carry on if the feed hasn't been updated for the specified amount of seconds. Otherwise it will stop. </p><p><b>It requires that the value passsed from previous process is a valid feed id (see <i>Get feed id</i>)</b></p>"), 'requireredis' => false, 'nochange' => true);
        $list[] = array(_("Input last update > secs, go to next"), ProcessArg::VALUE, "input_last_update_higher", 0, DataType::UNDEFINED, "Time", 'desc' => _("<p>Using the value of previous process as an input id, the execution of the processlist will carry on if the input hasn't been updated for the specified amount of seconds. Otherwise it will stop. </p><p><b>It requires that the value passsed from previous process is a valid input id (see <i>Get input id</i>)</b></p>"), 'requireredis' => false, 'nochange' => true);
        $list[] = array(_("Send email about a feed"), ProcessArg::TEXT, "send_email_feed", 0, DataType::UNDEFINED, "Notifications", 'desc' => _("<p>Send an email to the user with the specified body.</p><p>Supported template tags to customize body: {current_time}, {name}, {id}, {last_update}, {value}</p><p>Example body text: At {current_time}, the last update of {name} (feed id: {id}) was on {last_update} and it's value was {value}.</p><p><b>It requires that the value passsed from previous process is a valid feed id (see <i>Get feed id</i>)</b></p>"), 'requireredis' => false, 'nochange' => true);
        $list[] = array(_("Send email about an input"), ProcessArg::TEXT, "send_email_input", 0, DataType::UNDEFINED, "Notifications", 'desc' => _("<p>Send an email to the user with the specified body.</p><p>Supported template tags to customize body: {id}, {key}, {name}, {node}, {current_time}, {value}, {last_update}</p><p>Example body text: At {current_time} your input from node {node} with key {key} named {name} had value {value} and was last updated {last_update}.</p><p><b>It requires that the value passsed from previous process is a valid input id (see <i>Get input id</i>)</b></p>"), 'requireredis' => false, 'nochange' => true);
        return $list;
    }
    ...
    public function feed_last_update_higher($secs, $time, $value) { // $value must be feedid
    ...
    public function input_last_update_higher($secs, $time, $value) {// $value must be inputid
    ...
    public function send_email_input($emailbody, $time, $inputid) {// $inputid is the value passed from previous process,  it must be a valid feedid!!!
        global $user, $session;
